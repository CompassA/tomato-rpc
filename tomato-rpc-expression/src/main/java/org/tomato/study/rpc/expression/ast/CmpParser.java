...
@NoArgsConstructor
@AllArgsConstructor
public class CmpParser implements ExpressionParser {

    private AddAndSubParser addAndSubParser;

    @Override
    public ASTNode parse(TokenStream tokenStream) {
        ASTNode node = addAndSubParser.parse(tokenStream);
        if (node == null) {
            return null;
        }
        ...
        while (continueParse(tokenStream)) {
            ...
            ASTNode right = addAndSubParser.parse(tokenStream);
            if (right == null) {
                throw new IllegalStateException("illegal [CMP] expression, missing right node after " + opToken.getType());
            }
            node = new CmpNode(opToken, new ASTNode[] {node, right});
        }
        return node;
    }
